...
", problem.path_cost(self.path_cost, self.state, act, next) + problem.h(Node(next))"
...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        "para iterar el número de nodos visitados"
        numnodos=numnodos+1
        if problem.goal_test(node.state):
            "Si llegamos al final, imprimimos los nodos visitados"
            print("nodos visitados", numnodos, "y nodos expandidos", numnodos-1, "coste total", node.path_cost)
            return node
...
def R_A(problem):
    """Ramificación y acotración (RA)"""
    return graph_search(problem, ERREA())


def Busqueda_Heuristica(problem):
